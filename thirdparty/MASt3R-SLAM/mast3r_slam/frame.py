...
class Frame:
    ...
    def update_pointmap(self, X: torch.Tensor, C: torch.Tensor):
        ...
        # Original behavior: no confidence masking
        if self.N == 0:
...
class SharedKeyframes:
    ...
    def append(self, value: Frame):
        with self.lock:
            # If buffer is full, implement sliding window: remove oldest keyframe
            if self.n_size.value >= self.buffer:
                # Shift all data left by one position (remove oldest)
                # This is a simple sliding window: keep only the last (buffer-1) keyframes
                # Use in-place operations to avoid unnecessary cloning where possible
                for i in range(self.buffer - 1):
                    self.dataset_idx[i].copy_(self.dataset_idx[i + 1])
                    self.img[i].copy_(self.img[i + 1])
                    self.uimg[i].copy_(self.uimg[i + 1])
                    self.img_shape[i].copy_(self.img_shape[i + 1])
                    self.img_true_shape[i].copy_(self.img_true_shape[i + 1])
                    self.T_WC[i].copy_(self.T_WC[i + 1])
                    self.X[i].copy_(self.X[i + 1])
                    self.C[i].copy_(self.C[i + 1])
                    self.N[i].copy_(self.N[i + 1])
                    self.N_updates[i].copy_(self.N_updates[i + 1])
                    self.feat[i].copy_(self.feat[i + 1])
                    self.pos[i].copy_(self.pos[i + 1])
                    self.is_dirty[i].copy_(self.is_dirty[i + 1])
                # Set n_size to buffer - 1, so the new frame goes at position buffer - 1
                self.n_size.value = self.buffer - 1
